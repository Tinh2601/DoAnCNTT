...
public class Analytics1Month extends HttpServlet {
	...
	protected void AnalyticsYear(HttpServletRequest request, HttpServletResponse response)
		...
		if(value.equals("true")) {
			...
			title = "Total money in "+Month+"-"+Year;
			...
			for(int i=0;i<DayForChart;i++) {
				...
				System.out.println(day);
				System.out.println(YearAndMonth + day);
				total.add(billdao.tien(YearAndMonth + day));				
				name.add(day);	
			}
			SetAttribute(total, name, title, column_properities, request, response);
		}
...
//		request.setAttribute("table", "['JSP & Servlet', 20], ['Spring',12],['Struts',7]");
		String tableValue = "";
		int size = name.size();
